...
```
The conclusion is 0 because our test statistic is 2.64 which is larger than -1.64
...
test=list()
for (i in 1:100){
test[i] = (mean(l[[i]])-120)/(sd(l[[i]])/length(l[[i]]))
}
```
...
```{estions: use a for loop to loop over the 100 samples and create a variable using the function ifelse() to keep track of your 1’s and 0’s.
